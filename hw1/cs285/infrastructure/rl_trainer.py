...
class RL_Trainer(object):
    ...
    def run_training_loop(self, n_iter, collect_policy: MLPPolicySL, eval_policy: MLPPolicySL,
                          ...
                          start_relabel_with_expert=1, expert_policy: LoadedGaussianPolicy = None):
        """
    ...
    def collect_training_trajectories(
        ...
        # 처음에 iteration을 하나도 진행 안한상황! -> buffer에 가지고있는 expert data를 모두 넣는다
        if itr == 0:
            with open(load_initial_expertdata, 'rb') as f:
                loaded_paths = pickle.load(f)
            return loaded_paths, 0, None

            # (2) collect `self.params['batch_size']` transitions -> 이게 두번째 투두
    ...
    def do_relabel_with_expert(self, expert_policy: LoadedGaussianPolicy, paths: []):
