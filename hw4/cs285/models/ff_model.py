...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        # TODO Done (Q1) get numpy array of the predicted next-states (s_t+1)
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
        next_obs_pred, _ = self.forward(obs, acs, data_statistics['obs_mean'], data_statistics['obs_std'],
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :return:
        """
        # TODO Done (Q1) compute the normalized target for the model.
        ...
        # TODO Done (Q1) compute the loss
        _, pred_delta_normalized = self.forward(observations, actions, data_statistics['obs_mean'],
                                                data_statistics['obs_std'], data_statistics['acs_mean'],
                                                data_statistics['acs_std'], data_statistics['delta_mean'],
                                                data_statistics['delta_std'])

        loss = self.loss(pred_delta_normalized, target)
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
